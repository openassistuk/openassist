...
- Retrospective: the most important implementation catch was that detached PR handling had to be enforced both in the human upgrade plan and in the shared lifecycle report. The first pass only updated the plan, and the added integration test caught the missing `currentBranch` wiring before the branch was published.
