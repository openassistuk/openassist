...
- once linked, Codex chat requests preserve the upstream conversation contract by sending the runtime session id, account header, top-level instructions payload, and the upstream-aligned `/responses` fields Codex currently requires, including `store=false` and a prompt-cache key derived from the runtime session id
