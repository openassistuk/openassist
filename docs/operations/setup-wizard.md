...
- Once the linked account is chat-ready, Codex chat requests now preserve the upstream conversation contract by sending the runtime session id, account header, top-level instructions, and the upstream-aligned `/responses` fields Codex currently requires such as `store=false` and a prompt-cache key derived from the session id. If auth is chat-ready and chat still fails, treat that as a provider request issue rather than an auth issue.
