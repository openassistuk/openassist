...
- Once the linked account is chat-ready, Codex chat requests now preserve the upstream conversation contract by sending the runtime session id and account header expected by the Codex backend. If auth is chat-ready and chat still fails, treat that as a provider request issue rather than an auth issue.
